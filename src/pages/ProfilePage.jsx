...
export default function ProfilePage({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db,"users",user.uid), snap => {
      if (snap.exists()) { const d=snap.data(); setUserData(d); setName(d.name||""); setUsername(d.username||""); }
    });
  }, [user]);
  ...
  const checkUsername = async (val) => {
    ...
    if (clean === userData?.username) { setUsernameStatus("same"); return; }
  ...
  const saveProfile = async () => {
    ...
    if (!cleanUsername || cleanUsername.length < 3) { showMsg("ইউজারনেম কমপক্ষে ৩ অক্ষর"); return; }
    if (usernameStatus === "taken") { showMsg("ইউজারনেম নেওয়া হয়ে গেছে"); return; }
    setSaving(true);
    try {
      ...
      if (cleanUsername !== userData?.username) {
        ...
        const snap = await getDocs(q);
        if (!snap.empty) { showMsg("ইউজারনেম নেওয়া হয়ে গেছে"); setSaving(false); return; }
      }
      await updateDoc(doc(db,"users",user.uid), { name:name.trim(), username:cleanUsername, avatar });
      ...
      <div style={{ margin:"14px 18px 0", ...glass, padding:18 }}>
        ...
        {editing && (
          <>
            ...
            <label style={{ fontSize:12, color:"rgba(255,255,255,0.5)", display:"block", marginBottom:6 }}>ইউজারনেম (@)</label>
            <input style={{ ...inp, marginBottom:usernameStatus&&usernameStatus!=="same"?4:14, borderColor:usernameStatus==="taken"?"rgba(248,113,113,0.5)":usernameStatus==="available"?"rgba(52,211,153,0.5)":"rgba(255,255,255,0.1)" }} value={username} onChange={e=>{ setUsername(e.target.value); checkUsername(e.target.value); }} placeholder="username" />
            {usernameStatus==="checking" && <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", marginBottom:14 }}>⏳ চেক হচ্ছে...</div>}
            {usernameStatus==="available" && <div style={{ fontSize:11, color:"#34d399", marginBottom:14 }}>✅ ইউজারনেম পাওয়া যাচ্ছে</div>}
            {usernameStatus==="taken" && <div style={{ fontSize:11, color:"#f87171", marginBottom:14 }}>❌ ইউজারনেম নেওয়া হয়ে গেছে</div>}
