...
export default function MatchDetailPage({ match, onBack }) {
  ...
  const mTeamSize = m.teamSize || (/squad|4v4/i.test(m.mode||"") ? 4 : /duo|2v2/i.test(m.mode||"") ? 2 : 1);
  const mTotalSlots = Math.ceil((m.maxPlayers||48) / mTeamSize);
  const filledSlots = m.participants?.length || 0;
  const isFull = filledSlots >= mTotalSlots;
  const slotLeft = mTotalSlots - filledSlots;
  const fillPct = (filledSlots/mTotalSlots)*100;
    ...
    <div style={{ fontFamily:font, paddingBottom:30 }}>
      ...
      <div style={{ margin:"0 18px 16px", display:"flex", gap:8 }}>
        {[
          ...
          { label:mTeamSize>1?"টিম/স্লট":t("players"), value:`${filledSlots}/${mTotalSlots}`, color:"#60a5fa" },
        ].map((s,i)=>(
